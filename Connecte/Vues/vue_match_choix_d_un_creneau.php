<!DOCTYPE html>		<!-- localhost/www/Php_MySql/Php_PoO/poo_2.9_TP_Matchs/Connecte/Vues/vue_match_choix_d_un_creneau.php -->
<html>
	<head>
...
<div class="Conteneur">
	...
	Pour le match de <a class='affichEquipe'><?php echo $team1->id()." - ".$team1->nom()."</a> contre <a class='affichEquipe'>".$team2->id()." - ".$team2->nom()."</a>"; ?><br />
			...
			<form action="module_match_creation.php" method="post">
						...
						<input type="hidden" name="horaire" id="horaire1" />
			...
			<form action="module_match_creation.php" method="post">
						...
						<input type="hidden" name="id_equipe1" value=<?php echo $team1->id(); ?> />
						<input type="hidden" name="id_equipe2" value=<?php echo $team2->id(); ?> />
						<input type="hidden" name="horaire" id="horaire2" />
						<input type="submit" value="valider" />
			</form>
...
<script>
			...
			var horaire2 = document.getElementById("horaire2");
			var horaire1 = document.getElementById("horaire1");
			var creneau_match_dans_le_passe = document.getElementById("creneau_match_dans_le_passe");

			horaire1.value = creneau_match.value;
			horaire2.value = creneau_match_dans_le_passe.value;
...
function construireListes(){
			...
			creneau_match.options[0] = new Option("choisir un créneau", 0);
			premiereFois = true; 
			...
			for(i=0; i<=12; i++)	// la liste déroulante contiendra (35 jours de propositions) - (matchs en bdd)
			{
				for(j=8; j <= 20; j = j+2)
				{
					...
					if((dateCourante.getMonth() + 1).length ==1)
					{
						dateCouranteMois = "0" + (dateCourante.getMonth() + 1);						
					}else{dateCouranteMois = (dateCourante.getMonth() + 1);}

					if((dateCourante.getDate()).length ==1)
					{
						dateCouranteJour = "0" + (dateCourante.getDate());						
					}else{dateCouranteJour = (dateCourante.getDate());}

					if((dateCourante.getHours()).length ==1)
					{
						dateCouranteHeures = "0" + (dateCourante.getHours());						
					}else{dateCouranteHeures = (dateCourante.getHours());}
 
 					dateValeur = dateCourante.getFullYear() + "-" + dateCouranteMois+ "-" + dateCouranteJour + " " + 
				dateCouranteHeures + ":00:00";

				/*	dateValeur = dateCourante.getFullYear() + "-" + (dateCourante.getMonth() + 1) + "-" + dateCourante.getDate() + " " + 
...
//      *************************  SENS INVERSE
			...
			creneau_match_dans_le_passe.options[0] = new Option("", 3);
			compteur = 1;
			premiereFois = true;

			for(i=0; i<=365; i++)	// la liste déroulante contiendra (35 jours de propositions) - (matchs en bdd)
			{
				for(j=20 ; j >=8 ; j = j-2)
				{
					if(premiereFois)
		...
		/*			dateValeur = dateCourantePourSensInverse.getFullYear() + "-" + dateCourantePourSensInverse.getMonth() + "-" + dateCourantePourSensInverse.getDate() + " " + 
				...
				if((dateCourantePourSensInverse.getMonth() + 1).length ==1)
					{
						dateCouranteMois = "0" + (dateCourantePourSensInverse.getMonth() + 1);						
					}else{dateCouranteMois = (dateCourantePourSensInverse.getMonth() + 1);}

					if((dateCourante.getDate()).length ==1)
					{
						dateCouranteJour = "0" + (dateCourantePourSensInverse.getDate());						
					}else{dateCouranteJour = (dateCourantePourSensInverse.getDate());}

					if((dateCourante.getHours()).length ==1)
					{
						dateCouranteHeures = "0" + (dateCourantePourSensInverse.getHours());						
					}else{dateCouranteHeures = (dateCourantePourSensInverse.getHours());}
 
 					dateValeur = dateCourantePourSensInverse.getFullYear() + "-" + dateCouranteMois+ "-" + dateCouranteJour + " " + 
				dateCouranteHeures + ":00:00";
		...
		j5C1.addEventListener("click", function() {			upDateListes(); },false); 
			...
			j7C1.addEventListener("click", function() {			upDateListes(); },false); 
				...
				creneau_match.addEventListener('change', function(){
					horaire1.value = creneau_match.value; //alert('1');
					testB.innerHTML = horaire1.value;
				},false);

				creneau_match_dans_le_passe.addEventListener('change', function(){
					horaire2.value = creneau_match_dans_le_passe.value; //alert('2');
					testB.innerHTML = horaire2.value;
				},false);
